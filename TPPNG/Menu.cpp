...
updateMenu()
{
	...
	{
		string output = "CODIFICANDO IMAGENES...";
...
void Menu::
encodeImage(bool coder(Image& img, unsigned int threshold), int inputT)
{
	bool ret = true;
	...
	if (!(ret))
	{
		err.errorNum = FUNC_ERROR;
		err.detail = "Error en la compresion de imagenes!";
	}
}
